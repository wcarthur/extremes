...
plotpath <- "X:/georisk/HaRIA_B_Wind/data/derived/obs/daily_max_wind/wind/gammagpd"
...
for (file in filelist) {
  ...
  result <- tryCatch(fgammagpd(gust, useq = qs, fixedu = FALSE),
  ...
  try(with(m.gammagpd,
           lines(xp,
                 dgammagpd(xp, gshape, gscale, u, xi),
                 col = "red", lwd = 2)))
