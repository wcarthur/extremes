...
def epplot(gust, paramdf, stnname, plotfile):
    """
    Plot the exceedance probabiity curve for all thresholds that produce a
    bounded curve (i.e. xi < 0).

    We add a red curve for the parameters that correspond to the median
    shape value, a blue curve for the parameters corresponding to the median
    threshold
    Args:
        gust (_type_): _description_
        paramdf (_type_): _description_
        stnname (_type_): _description_
        plotfile (_type_): _description_
    """
    n = len(gust)
    empprob = ppoints(n)
...
def fitGammaGPD(stnnum, stnname, inputPath, outputPath, jitter=False):
    ...
    tlower = np.quantile(gust, 0.5)
    tupper = np.max(gust)
    paramdf = pd.DataFrame(columns=['t', 'gshape', 'gscale', 'u', 'sigmau', 'xi', 'phiu', 'sigmase', 'xise'])
    for t in np.arange(tlower, tupper, 0.05):
        if len(gust[gust > t]) < 5:
            print("Less than 5 records greater than the threshold")
            break
        fit = evmix.fgammagpd(jgust, useq=t, std_err=True,)
        if fit.rx2('xi')[0] < 0:
            if isinstance(fit.rx2('se'), np.ndarray):
                sigmase = fit.rx2('se')[2]
                xise = fit.rx2('se')[3]
            else:
                sigmase = 0
                xise = 0
