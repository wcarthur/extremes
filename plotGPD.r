plotGPD = function(a, u, la, n, npy, mat, dat, xdat, xlabel="",ylabel="",plotpoints=FALSE)
{
    ...
    plot(m/npy, q, log = "x", type = "n", xlim = c(1, max(m)/npy),ylim = c(u, max(xdat, q[q > u - 1] + 1.96 * sqrt(v)[q >u - 1])), xlab = xlabel, ylab = ylabel,main = "Return Level Plot")
    grid(equilogs=FALSE)
    ...
    if(plotpoints==TRUE){
        nl <- n - length(dat) + 1
        sdat <- sort(xdat)
        points((1/(1 - (1:n)/(n + 1))/npy)[(1/(1 - (1:n)/(n + 1))/npy)>1], sdat[(1/(1 - (1:n)/(n + 1))/npy) > 1])
    }
}
...
plotGPD_CI = function(dataset,yrs,threshold,RP,confInt=0.95,plotObs=FALSE)
{
    ...
    plot(RP,gpdfit[,3],log="x",type="l",ylab="Wind speed (m/s)",xlab="Return Period (yrs)",xlim=c(1,10000),ylim=c(10,ymax))
