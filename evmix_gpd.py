...
def histplot(gust, paramdf, stnname, plotfile):
    ...
    dfitxi = evmix.dgpd(xx, pmedxi.u, pmedxi.sigmau, pmedxi.xi, pmedxi.phiu)
    dfitu = evmix.dgpd(xx, pmedu.u, pmedu.sigmau, pmedu.xi, pmedu.phiu)
...
def epplot(gust, paramdf, stnname, plotfile):
    ...
    for idx, row in paramdf.iterrows():
        thequant = evmix.qgpd(theprob, row.u, row.sigmau, row.xi, row.phiu)
    ...
    medquant = evmix.qgpd(theprob, pmed.u, pmed.sigmau, pmed.xi, pmed.phiu)
    ...
    if plotse:
        ...
        minquant = evmix.qgpd(theprob, pmin.u, pmin.sigmau, pmin.xi, pmin.phiu)
    ...
    plt.savefig(plotfile, bbox_inches="tight")
    plt.close(fig)
    np.savetxt(plotfile.replace("png", "csv"),
...
def calcMAE(gust, t, fit):
    ...
    empcdf = ECDF(sgust)
    fgpd = gpd(fit)
    dfit = evmix.dgpd(sgust, fgpd.u, fgpd.sigmau, fgpd.xi, fgpd.phiu)
