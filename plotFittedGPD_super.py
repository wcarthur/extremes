...
def plotFitDataFrame(data, mu, xi, sigma, title):
    """
    Plot a fitted distribution, with approximate 90% confidence interval
    and empirical return period values.

    :param data: :class:`pandas.DataFrame` of observed data values.
    :param float mu: Selected threshold value.
    :param float xi: Fitted shape parameter.
    :param float sigma: Fitted scale parameter.
    :param str title: Title string for the plot.
    ...
    rp = np.array([1, 2, 5, 10, 20, 50, 100, 200,
                   500, 1000, 2000, 5000, 10000])
    values = data['gust']
    rate = float(len(values[values > mu])) / float(len(values))
    rval = returnLevels(rp, mu, xi, sigma, rate)

    data['emprp'] = empReturnPeriod(values)
    err = returnPeriodUncertainty(values, mu, xi, sigma, rp)

    sortedmax = data['gust'].values
    stns = data['station'].values
    data['color'] = pd.cut(data['station'], [-1, 0, 31011, 32040, 33119],
    ...
    ax1.semilogx(rp, rval + 1.96 * err, label="95% CI",
                linestyle='--', color='0.5')
    ax1.semilogx(rp, rval - 1.96 * err, linestyle='--', color='0.5')
    stationNames = ['Cairns', 'Townsville', 'Mackay']
    for s,sn, m in zip(data.station.unique()[1:], stationNames, ['o', '*', 'x']):
        print(f"{s}: {sn}")
        tempdf = data[(data.emprp > 1) & (data.station==s)]
        ax1.scatter(tempdf.emprp, tempdf.gust,
                    c=tempdf.color, marker=m,
                    label=sn,zorder=100)
...
df['gust'] = 1.098 * df.gust #(assumed 3-second gust to 0.2 second gust conversion)
timerange = 187 #df.datetime.max().year - df.datetime.min().year + 1
xi, sigma, mu = selectThreshold(df.gust) #, start=np.min(df.gust))
dummydata = np.zeros(int(timerange)*365)
ndata = len(df.gust)
dd = pd.DataFrame(index=np.arange(len(dummydata)), columns=['station', 'gust'])
dd.loc[-ndata:, :] = df[['station', 'gust']].sort_values('gust')
dd.sort_values('gust', na_position='first', inplace=True)
dd.fillna(0, inplace=True)


plotFitDataFrame(dd, mu, xi, sigma, "North Queensland")
